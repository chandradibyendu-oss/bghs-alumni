import { NextRequest, NextResponse } from 'next/server'
import { createClient } from '@supabase/supabase-js'
import crypto from 'crypto'

const supabase = createClient(
  process.env.NEXT_PUBLIC_SUPABASE_URL!,
  process.env.SUPABASE_SERVICE_ROLE_KEY!
)

export async function POST(request: NextRequest) {
  try {
    const { email, phone } = await request.json()

    if (!email && !phone) {
      return NextResponse.json(
        { error: 'Email or phone number is required' },
        { status: 400 }
      )
    }

    // Helper to normalize phone input
    const normalizePhone = (raw: string): string[] => {
      const trimmed = raw.trim()
      const digits = trimmed.replace(/\D/g, '')
      const plusDigits = digits.startsWith('+') ? digits : `+${digits}`
      // Return unique candidates preserving original
      const set = new Set<string>([trimmed, digits, plusDigits])
      return Array.from(set).filter(Boolean)
    }

    // Check if user exists in profiles table first
    let user: { id: string; email: string | null; phone: string | null } | null = null
    if (email) {
      const { data: emailUser } = await supabase
        .from('profiles')
        .select('id, email, phone')
        .eq('email', email.trim())
        .single()
      if (emailUser) user = emailUser as any
    }

    if (!user && phone) {
      const candidates = normalizePhone(String(phone))
      for (const candidate of candidates) {
        const { data: phoneUser } = await supabase
          .from('profiles')
          .select('id, email, phone')
          .eq('phone', candidate)
          .single()
        if (phoneUser) { user = phoneUser as any; break }
      }
    }

    // If not found in profiles, check auth.users (Supabase Auth)
    if (!user && (email || phone)) {
      const { data: authUsers, error: authError } = await supabase.auth.admin.listUsers()
      if (!authError && authUsers) {
        let authUser = null as any
        if (email) {
          authUser = authUsers.users.find(u => u.email === email.trim())
        }
        if (!authUser && phone) {
          const candidates = normalizePhone(String(phone))
          authUser = authUsers.users.find(u => u.phone && candidates.includes(u.phone))
        }
        if (authUser) {
          user = {
            id: authUser.id,
            email: authUser.email,
            phone: authUser.phone || null
          }
        }
      }
    }

    if (!user) {
      return NextResponse.json(
        { error: 'No account found with the provided email or phone number' },
        { status: 404 }
      )
    }

    // Generate 6-digit OTP
    const otp = Math.floor(100000 + Math.random() * 900000).toString()
    const expiresAt = new Date(Date.now() + 10 * 60 * 1000) // 10 minutes

    // Hash the OTP for secure storage
    const otpHash = crypto.createHash('sha256').update(otp).digest('hex')

    // Store OTP in database
    const { error: insertError } = await supabase
      .from('password_reset_otps')
      .insert({
        email: email || null,
        phone: phone || null,
        otp_hash: otpHash,
        expires_at: expiresAt.toISOString()
      })

    if (insertError) {
      console.error('OTP storage error:', insertError)
      return NextResponse.json(
        { error: 'Failed to store OTP' },
        { status: 500 }
      )
    }

    // Send OTP via email
    if (user.email) {
      await sendEmailOTP(user.email, otp)
    }

    // Send OTP via SMS (if phone is provided)
    if (user.phone) {
      await sendSMSOTP(user.phone, otp)
    }

    return NextResponse.json({
      message: 'OTP sent successfully',
      expires: expiresAt.getTime()
    })

  } catch (error) {
    console.error('Forgot password error:', error)
    return NextResponse.json(
      { error: 'Internal server error' },
      { status: 500 }
    )
  }
}

async function sendEmailOTP(email: string, otp: string) {
  try {
    const { sendEmail, generateOTPEmail } = await import('@/lib/email-service')
    
    const emailOptions = generateOTPEmail(otp)
    emailOptions.to = email
    
    const success = await sendEmail(emailOptions)
    
    if (!success) {
      throw new Error('Failed to send email')
    }
    
  } catch (error) {
    console.error('Email OTP error:', error)
    throw new Error('Failed to send email OTP')
  }
}

async function sendSMSOTP(phone: string, otp: string) {
  try {
    const { sendSMS, generateOTPSMS } = await import('@/lib/sms-service')
    
    const smsOptions = generateOTPSMS(otp)
    smsOptions.to = phone
    
    const success = await sendSMS(smsOptions)
    if (!success) {
      console.warn('SMS send reported unsuccessful; continuing with email-only flow')
    }
    
  } catch (error) {
    console.error('SMS OTP error:', error)
    // Do not fail the whole flow if SMS fails; email OTP may have been sent
  }
}

// Note: OTP verification/update helpers live in '@/lib/otp-utils'
