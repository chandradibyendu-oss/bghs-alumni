...
export default function EditNoticePage() {
    ...
    try {
      ...
      if (data) {
        ...
        setFormData({
          ...
          is_public: data.is_public !== false, // Default to true if null/undefined
      ...
      alert('Failed to load notice. It may not exist or you may not have permission to edit it.')
      router.push('/admin/notices')
    } finally {
      setLoading(false)
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { data: { user } } = await supabase.auth.getUser()
      if (!user) {
        alert('Please login to update notices')
        return
      }
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600 mt-1">Update notice details</p>
            </div>
          </div>
        </div>
      </div>
