...
export default function EventDetailPage({ params }: { params: { id: string } }) {
  ...
  const fetchEvent = async () => {
    try {
      ...
      const visibility = data?.metadata?.visibility || 'public'
      
      // Check access permissions
      let hasAccess = false
      if (visibility === 'public') {
        hasAccess = true
      } else if (visibility === 'alumni_only' || visibility === 'invite_only') {
      ...
      if (!hasAccess) {
        setError('You do not have permission to view this event. Please log in to access alumni-only events.')
        setEvent(null)
        return
      }
  ...
  if (error || !event) {
    const isAccessDenied = error?.includes('permission') || error?.includes('log in')
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center max-w-md mx-auto px-4">
          <h1 className="text-2xl font-bold text-gray-900 mb-4">
            {isAccessDenied ? 'Access Restricted' : 'Event Not Found'}
          </h1>
          <p className="text-gray-600 mb-6">
            {isAccessDenied 
              ? 'This event is only available to registered alumni members. Please log in to view and register for alumni-only events.'
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            ...
            <Link href="/events" className={isAccessDenied ? "btn-secondary" : "btn-primary"}>
              Back to Events
            </Link>
          </div>
        </div>
