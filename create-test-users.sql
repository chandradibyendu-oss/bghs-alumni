-- =====================================================
-- BGHS Alumni Website - Test Users Creation Script (Updated)
-- =====================================================
-- This script creates test users with different roles for testing
-- the dynamic role-based access control (RBAC) system.
-- 
-- IMPORTANT: Execute this script in your Supabase SQL editor
-- NOTE: This script works with dynamic permission lookup system
-- =====================================================

-- First, let's ensure we have the necessary roles
-- (These should already exist from previous setup, but let's verify)

-- Insert test users with different roles
-- Note: All users have password: 'TestPass123!'
-- All users are pre-approved for immediate testing
-- Permissions are now dynamically looked up from user_roles table

-- =====================================================
-- 1. SUPER ADMINISTRATOR
-- =====================================================
INSERT INTO profiles (
    id,
    email,
    first_name,
    last_name,
    phone,
    graduation_year,
    profession,
    company,
    city,
    state,
    country,
    is_approved,
    role,
    created_at,
    updated_at
) VALUES (
    '11111111-1111-1111-1111-111111111111',
    'superadmin@bghs-alumni.com',
    'Super',
    'Administrator',
    '+91-9876543210',
    1990,
    'System Administrator',
    'BGHS Alumni Association',
    'Mumbai',
    'Maharashtra',
    'India',
    true,
    'super_admin',
    NOW(),
    NOW()
) ON CONFLICT (id) DO UPDATE SET
    email = EXCLUDED.email,
    first_name = EXCLUDED.first_name,
    last_name = EXCLUDED.last_name,
    role = EXCLUDED.role,
    is_approved = EXCLUDED.is_approved;

-- =====================================================
-- 2. EVENT MANAGER
-- =====================================================
INSERT INTO profiles (
    id,
    email,
    first_name,
    last_name,
    phone,
    graduation_year,
    profession,
    company,
    city,
    state,
    country,
    is_approved,
    role,
    privacy_settings,
    created_at,
    updated_at
) VALUES (
    '22222222-2222-2222-2222-222222222222',
    'eventmanager@bghs-alumni.com',
    'Priya',
    'Sharma',
    '+91-9876543211',
    2005,
    'Event Coordinator',
    'Event Management Co.',
    'Delhi',
    'Delhi',
    'India',
    true,
    'event_manager',
    '{
        "show_in_directory": true,
        "show_email": true,
        "show_phone": true,
        "show_profession": true,
        "show_company": true,
        "show_city": true,
        "show_graduation_year": true,
        "show_linkedin": true,
        "show_facebook": false,
        "show_instagram": false,
        "show_twitter": false,
        "show_website": true,
        "show_bio": true,
        "show_achievements": true
    }'::jsonb,
    NOW(),
    NOW()
) ON CONFLICT (id) DO UPDATE SET
    email = EXCLUDED.email,
    first_name = EXCLUDED.first_name,
    last_name = EXCLUDED.last_name,
    role = EXCLUDED.role,
    is_approved = EXCLUDED.is_approved;

-- =====================================================
-- 3. CONTENT CREATOR
-- =====================================================
INSERT INTO profiles (
    id,
    email,
    first_name,
    last_name,
    phone,
    graduation_year,
    profession,
    company,
    city,
    state,
    country,
    is_approved,
    role,
    privacy_settings,
    created_at,
    updated_at
) VALUES (
    '33333333-3333-3333-3333-333333333333',
    'contentcreator@bghs-alumni.com',
    'Rahul',
    'Patel',
    '+91-9876543212',
    2010,
    'Content Writer',
    'Digital Media Agency',
    'Bangalore',
    'Karnataka',
    'India',
    true,
    'content_creator',
    '{
        "show_in_directory": true,
        "show_email": true,
        "show_phone": false,
        "show_profession": true,
        "show_company": true,
        "show_city": true,
        "show_graduation_year": true,
        "show_linkedin": true,
        "show_facebook": true,
        "show_instagram": true,
        "show_twitter": true,
        "show_website": true,
        "show_bio": true,
        "show_achievements": true
    }'::jsonb,
    NOW(),
    NOW()
) ON CONFLICT (id) DO UPDATE SET
    email = EXCLUDED.email,
    first_name = EXCLUDED.first_name,
    last_name = EXCLUDED.last_name,
    role = EXCLUDED.role,
    is_approved = EXCLUDED.is_approved;

-- =====================================================
-- 4. CONTENT MODERATOR
-- =====================================================
INSERT INTO profiles (
    id,
    email,
    first_name,
    last_name,
    phone,
    graduation_year,
    profession,
    company,
    city,
    state,
    country,
    is_approved,
    role,
    privacy_settings,
    created_at,
    updated_at
) VALUES (
    '44444444-4444-4444-4444-444444444444',
    'moderator@bghs-alumni.com',
    'Anjali',
    'Singh',
    '+91-9876543213',
    2008,
    'Community Manager',
    'Social Media Solutions',
    'Pune',
    'Maharashtra',
    'India',
    true,
    'content_moderator',
    '{
        "show_in_directory": true,
        "show_email": true,
        "show_phone": true,
        "show_profession": true,
        "show_company": false,
        "show_city": true,
        "show_graduation_year": true,
        "show_linkedin": true,
        "show_facebook": false,
        "show_instagram": false,
        "show_twitter": false,
        "show_website": false,
        "show_bio": true,
        "show_achievements": false
    }'::jsonb,
    NOW(),
    NOW()
) ON CONFLICT (id) DO UPDATE SET
    email = EXCLUDED.email,
    first_name = EXCLUDED.first_name,
    last_name = EXCLUDED.last_name,
    role = EXCLUDED.role,
    is_approved = EXCLUDED.is_approved;

-- =====================================================
-- 5. DONATION MANAGER
-- =====================================================
INSERT INTO profiles (
    id,
    email,
    first_name,
    last_name,
    phone,
    graduation_year,
    profession,
    company,
    city,
    state,
    country,
    is_approved,
    role,
    privacy_settings,
    created_at,
    updated_at
) VALUES (
    '55555555-5555-5555-5555-555555555555',
    'donationmanager@bghs-alumni.com',
    'Vikram',
    'Gupta',
    '+91-9876543214',
    2000,
    'Fundraising Manager',
    'Non-Profit Organization',
    'Chennai',
    'Tamil Nadu',
    'India',
    true,
    'donation_manager',
    '{
        "show_in_directory": true,
        "show_email": true,
        "show_phone": true,
        "show_profession": true,
        "show_company": true,
        "show_city": true,
        "show_graduation_year": true,
        "show_linkedin": true,
        "show_facebook": false,
        "show_instagram": false,
        "show_twitter": false,
        "show_website": true,
        "show_bio": true,
        "show_achievements": true
    }'::jsonb,
    NOW(),
    NOW()
) ON CONFLICT (id) DO UPDATE SET
    email = EXCLUDED.email,
    first_name = EXCLUDED.first_name,
    last_name = EXCLUDED.last_name,
    role = EXCLUDED.role,
    is_approved = EXCLUDED.is_approved;

-- =====================================================
-- 6. ALUMNI PREMIUM MEMBER
-- =====================================================
INSERT INTO profiles (
    id,
    email,
    first_name,
    last_name,
    phone,
    graduation_year,
    profession,
    company,
    city,
    state,
    country,
    is_approved,
    role,
    privacy_settings,
    created_at,
    updated_at
) VALUES (
    '66666666-6666-6666-6666-666666666666',
    'premiummember@bghs-alumni.com',
    'Sneha',
    'Joshi',
    '+91-9876543215',
    2015,
    'Software Engineer',
    'Tech Solutions Ltd.',
    'Hyderabad',
    'Telangana',
    'India',
    true,
    'alumni_premium',
    '{
        "show_in_directory": true,
        "show_email": true,
        "show_phone": true,
        "show_profession": true,
        "show_company": true,
        "show_city": true,
        "show_graduation_year": true,
        "show_linkedin": true,
        "show_facebook": true,
        "show_instagram": true,
        "show_twitter": true,
        "show_website": true,
        "show_bio": true,
        "show_achievements": true
    }'::jsonb,
    NOW(),
    NOW()
) ON CONFLICT (id) DO UPDATE SET
    email = EXCLUDED.email,
    first_name = EXCLUDED.first_name,
    last_name = EXCLUDED.last_name,
    role = EXCLUDED.role,
    is_approved = EXCLUDED.is_approved;

-- =====================================================
-- 7. ALUMNI MEMBER (STANDARD)
-- =====================================================
INSERT INTO profiles (
    id,
    email,
    first_name,
    last_name,
    phone,
    graduation_year,
    profession,
    company,
    city,
    state,
    country,
    is_approved,
    role,
    privacy_settings,
    created_at,
    updated_at
) VALUES (
    '77777777-7777-7777-7777-777777777777',
    'alumnimember@bghs-alumni.com',
    'Arjun',
    'Kumar',
    '+91-9876543216',
    2018,
    'Marketing Executive',
    'Advertising Agency',
    'Kolkata',
    'West Bengal',
    'India',
    true,
    'alumni_member',
    '{
        "show_in_directory": true,
        "show_email": true,
        "show_phone": false,
        "show_profession": true,
        "show_company": true,
        "show_city": true,
        "show_graduation_year": true,
        "show_linkedin": true,
        "show_facebook": false,
        "show_instagram": false,
        "show_twitter": false,
        "show_website": false,
        "show_bio": true,
        "show_achievements": false
    }'::jsonb,
    NOW(),
    NOW()
) ON CONFLICT (id) DO UPDATE SET
    email = EXCLUDED.email,
    first_name = EXCLUDED.first_name,
    last_name = EXCLUDED.last_name,
    role = EXCLUDED.role,
    is_approved = EXCLUDED.is_approved;

-- =====================================================
-- 8. PENDING APPROVAL USER
-- =====================================================
INSERT INTO profiles (
    id,
    email,
    first_name,
    last_name,
    phone,
    graduation_year,
    profession,
    company,
    city,
    state,
    country,
    is_approved,
    role,
    privacy_settings,
    created_at,
    updated_at
) VALUES (
    '88888888-8888-8888-8888-888888888888',
    'pendinguser@bghs-alumni.com',
    'Rohit',
    'Verma',
    '+91-9876543217',
    2020,
    'Recent Graduate',
    'Startup Company',
    'Ahmedabad',
    'Gujarat',
    'India',
    false,
    'alumni_member',
    '{
        "show_in_directory": false,
        "show_email": false,
        "show_phone": false,
        "show_profession": false,
        "show_company": false,
        "show_city": false,
        "show_graduation_year": false,
        "show_linkedin": false,
        "show_facebook": false,
        "show_instagram": false,
        "show_twitter": false,
        "show_website": false,
        "show_bio": false,
        "show_achievements": false
    }'::jsonb,
    NOW(),
    NOW()
) ON CONFLICT (id) DO UPDATE SET
    email = EXCLUDED.email,
    first_name = EXCLUDED.first_name,
    last_name = EXCLUDED.last_name,
    role = EXCLUDED.role,
    is_approved = EXCLUDED.is_approved;

-- =====================================================
-- 9. PRIVATE PROFILE USER
-- =====================================================
INSERT INTO profiles (
    id,
    email,
    first_name,
    last_name,
    phone,
    graduation_year,
    profession,
    company,
    city,
    state,
    country,
    is_approved,
    role,
    privacy_settings,
    created_at,
    updated_at
) VALUES (
    '99999999-9999-9999-9999-999999999999',
    'privateuser@bghs-alumni.com',
    'Kavya',
    'Reddy',
    '+91-9876543218',
    2012,
    'Data Scientist',
    'AI Research Lab',
    'Bangalore',
    'Karnataka',
    'India',
    true,
    'alumni_member',
    '{
        "show_in_directory": false,
        "show_email": false,
        "show_phone": false,
        "show_profession": false,
        "show_company": false,
        "show_city": false,
        "show_graduation_year": false,
        "show_linkedin": false,
        "show_facebook": false,
        "show_instagram": false,
        "show_twitter": false,
        "show_website": false,
        "show_bio": false,
        "show_achievements": false
    }'::jsonb,
    NOW(),
    NOW()
) ON CONFLICT (id) DO UPDATE SET
    email = EXCLUDED.email,
    first_name = EXCLUDED.first_name,
    last_name = EXCLUDED.last_name,
    role = EXCLUDED.role,
    is_approved = EXCLUDED.is_approved;

-- =====================================================
-- VERIFICATION QUERY
-- =====================================================
-- Run this query to verify all test users were created successfully
SELECT 
    email,
    first_name,
    last_name,
    role,
    is_approved,
    registration_id,
    CASE 
        WHEN privacy_settings->>'show_in_directory' = 'true' THEN 'Visible'
        ELSE 'Hidden'
    END as directory_visibility
FROM profiles 
WHERE email LIKE '%@bghs-alumni.com'
ORDER BY 
    CASE role
        WHEN 'super_admin' THEN 1
        WHEN 'event_manager' THEN 2
        WHEN 'content_creator' THEN 3
        WHEN 'content_moderator' THEN 4
        WHEN 'donation_manager' THEN 5
        WHEN 'alumni_premium' THEN 6
        WHEN 'alumni_member' THEN 7
        ELSE 8
    END;

-- =====================================================
-- NOTES FOR TESTING
-- =====================================================
-- 1. All users have the password: TestPass123!
-- 2. All approved users will get registration IDs automatically
-- 3. Pending users will not appear in directory until approved
-- 4. Private users have all privacy settings set to false
-- 5. Each user has different privacy settings for testing
-- 6. Use these credentials to test different role-based access
