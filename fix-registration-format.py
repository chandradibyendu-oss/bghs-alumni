#!/usr/bin/env python3
"""
Fix registration number format to BGHSA-2025-XXXXX
"""

import pandas as pd

def create_corrected_alumni_csv_with_proper_registration():
    """Create corrected CSV with proper BGHSA-2025-XXXXX registration format"""
    ...
    # Save to CSV
    output_path = "bengali_alumni_final_with_registration.csv"
    df.to_csv(output_path, index=False, encoding='utf-8')
    
    print(f"Final CSV file with correct registration format saved to: {output_path}")
    print(f"Total records: {len(df)}")
    print(f"Deceased records: {len(df[df['Is Deceased'] == 'true'])}")
    print(f"Living records: {len(df[df['Is Deceased'] == 'false'])}")
    
    # Show sample records with correct registration numbers
    print("\nSample records with correct BGHSA-2025-XXXXX format:")
    ...
    print("\nRegistration number conversion examples (BGHSA-2025-XXXXX):")
...
if __name__ == "__main__":
    create_corrected_alumni_csv_with_proper_registration()
