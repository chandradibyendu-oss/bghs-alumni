-- ========================================
-- BGHS ALUMNI PAYMENT SYSTEM - SAFE INSTALLATION
-- ========================================
-- This script handles BOTH scenarios:
--   1. Fresh install (no payment tables exist)
--   2. Migration from old schema (tables exist with old column names)
--
...
-- ========================================
-- PART 0: CLEANUP OLD INCONSISTENT DATA
-- ========================================

-- If payment_configurations table exists with old 'category' column, migrate it
DO $$ 
BEGIN
    -- Check if table exists with old column name
    IF EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'payment_configurations' 
        AND column_name = 'category'
    ) THEN
        -- Rename old column to new standard name
        ...
        ALTER TABLE payment_configurations ADD CONSTRAINT payment_configurations_payment_category_check 
            CHECK (payment_category IN ('registration', 'event', 'donation', 'membership', 'other'));
        
        RAISE NOTICE 'Migrated payment_configurations table to new schema';
    END IF;
END $$;
...
--
-- Expected: 5 tables (payment_configurations, payment_transactions, 
--                      payment_receipts, payment_notification_queue, payment_tokens)
-- ========================================
